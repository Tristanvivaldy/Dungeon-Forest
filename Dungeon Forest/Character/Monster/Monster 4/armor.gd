...
const DeathEffect = preload("res://Effect/Monster 3/DeathEffect.tscn")
	...
	match state:
		...
		CHASE:
			...
			if (player != null):
				...
				if((player.global_position.x - self.global_position.x >= -41 and player.global_position.x - self.global_position.x <= 37) and (player.global_position.y - self.global_position.y >= -31 and player.global_position.y - self.global_position.y <= 51)):
					sprite.play("attack1")
			else :
				state = IDLE
...
func _on_Stats_no_health():
	playerstats.gain_experience(75)
	queue_free()
