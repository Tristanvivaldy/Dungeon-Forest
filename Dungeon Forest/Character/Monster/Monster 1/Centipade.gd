...
const DeathEffect = preload("res://Effect/Monster 1/DeathEffect.tscn")
	...
	match state:
		...
		CHASE:
			...
			if (player != null):
				...
				if((player.global_position.x - self.global_position.x >= -31 and player.global_position.x - self.global_position.x <= 27) and (player.global_position.y - self.global_position.y >= -21 and player.global_position.y - self.global_position.y <= 41)):
					sprite.play("att")
			else :
				state = IDLE
